...
#define MAX_UCHAR	0xff // 16진수 256
...
void printHex()
{
	printf("hex: char(%x), short(%x), int(%x), uchar(%x), ushort(%x), uint(%x)\n",
		MAX_CHAR, MAX_SHORT, MAX_INT, MAX_UCHAR, MAX_USHORT, MAX_UINT);
}
...
void printUnsignedChar()
{
	...
	printf("int(%d)(%u), unsigned int(%d)(%u)\n", sx+1, sx, ux);
}
...
void printWidth()
{
	 ...
	 * 3. 실수(%f)는 자리수에 소숫점이 포함, 소숫점 이하는 기본 6자리
	 * 4. 실수(%15.10f)는 전체 자릿수는 15자리, 소숫점 이하는 10자리, 소숫점은 전체 자릿수에 포함된다
	 */
	
	printf("char(%4c), short(%5d), int(%5d), float(%7.6f), double(%15.10f)\n",
...
void printBasic()
{
	...
	// format: 실수형 -> %f는 기본적으로 수숫점 이하 6자리까지 출력 
	// format: 실수형 -> %.10f
